...
@pytest.mark.parametrize("dni_test", TEST_DNI_VALIDOS)
def test_check_dni_correcto(dni, dni_test):
    dni.setDni(dni_test)
    assert dni.checkDni()

@pytest.mark.parametrize("dni_test", TEST_DNI_FORMATO_ERRONEO)
...
@pytest.mark.parametrize("dni_test", TEST_DNI_LETRA_ERRONEA)
